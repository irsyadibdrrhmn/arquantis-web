      ...
      <div className="relative mx-auto max-w-6xl px-6">
        <div className="mb-16 flex items-center gap-4">
          ...
          <span className="font-mono text-[10px] tracking-[0.35em] uppercase text-[#c9a45c]">
            // {t("about", "sectionLabel").replace(/ /g, "_")}
          </span>
        ...
        <div className="grid lg:grid-cols-2 gap-14 items-center">
          <div>
            <h2 className="font-display text-3xl font-black leading-tight text-white md:text-4xl xl:text-5xl">
              {t("about", "heading")}{" "}
              <span className="text-gradient-gold">{t("about", "headingAccent")}</span>
            </h2>
            <p className="mt-6 text-base leading-8 text-[#7a8fa6]">{t("about", "p1")}</p>
            <p className="mt-4 text-base leading-8 text-[#7a8fa6]">
              {t("about", "p2")}{" "}
              <span className="text-[#c9a45c] font-semibold">{t("about", "p2Accent")}</span>{" "}
              {t("about", "p2b")}
            </p>
            <p className="mt-4 text-base leading-8 text-[#7a8fa6]">
              {t("about", "p3")}{" "}
              <span className="text-[#00e5ff] font-semibold">{t("about", "p3Accent")}</span>
              {t("about", "p3b")}
            </p>
            <div className="mt-8 inline-flex items-center gap-3 rounded-xl border border-[rgba(201,164,92,0.3)] bg-[rgba(201,164,92,0.05)] px-5 py-3 group cursor-pointer hover:border-[rgba(201,164,92,0.6)] hover:bg-[rgba(201,164,92,0.1)] transition-all">
              <span className="font-mono text-xs tracking-[0.15em] uppercase text-[#c9a45c]">{t("about", "cta")}</span>
            ...
            <div className="relative grid grid-cols-2 gap-4">
              {stats.map((item, i) => (
                <div key={i} className="glass-card rounded-2xl p-6 card-hover group relative">
                  <div className="font-display text-3xl font-black md:text-4xl" style={{ color: item.color, textShadow: `0 0 20px ${item.color}60` }}>
                    {item.value}
                  </div>
                  <div className="mt-3 font-mono text-[10px] tracking-[0.2em] uppercase text-[#7a8fa6]">
                    {t("about", item.labelKey)}
                  </div>
                  <div className="absolute top-3 right-3 h-1.5 w-1.5 rounded-full opacity-60" style={{ backgroundColor: item.color, boxShadow: `0 0 6px ${item.color}` }} />
                </div>
              ))}
            </div>
          </div>
